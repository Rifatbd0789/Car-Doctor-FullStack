...
const verifyToken = async (req, res, next) => {
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
    if (err) {
      return res.status(401).send({ message: "unauthorized after verify" });
    }
    req.user = decoded;
    next();
  });
...
async function run() {
  try {
    ...
    // send Bookings from client side to server
...
app.listen(port, () => {
  console.log(`Cars Doctor Server on ${port}`);
});
